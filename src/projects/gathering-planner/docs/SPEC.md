...
## 화면 구성
- **16:9 무대:** PC에서는 앱 전체가 **고정 16:9 무대**(가운데 정렬). 창이 다른 비율이면
  빈 배경이 양쪽에 균등하게 — **세로 긴 창은 위·아래, 가로 긴 창은 좌·우**가 **완전히 빈
  배경**(아무것도 없고 그리기도 안 됨). 모바일은 화면을 꽉 채움.
...
- **라이브 커서:** 각 참가자의 마우스를 닉네임 라벨과 함께 실시간 표시(낙서 16:9 영역
  기준). 모바일은 낙서 패널이 열려 있을 때 패널 안에서만 보임.
- **플래너 (데스크톱 우측 / 모바일 위):**
  - **날짜 투표:** 호스트가 정한 후보 날짜만 투표 가능(그 외 날짜는 흐리게).
    표수만큼 진해지는 파스텔 히트맵. 각 후보 날짜에 하트(내가 고른 날 ❤️, 아니면 🤍,
    장소 하트와 동일)와 표수 배지.
  - **장소 투표:** 입력(textarea)+추가, 둥근 카드 리스트, 하트 업보트/카운트, ✕ 삭제(확인).
    지도 공유(예: [네이버지도]\n가게명\n주소\nURL)를 붙여넣으면 가게명·주소·클릭 가능한
    "지도 열기" 링크로 파싱해 표시. 일반 링크도 자동 클릭 가능.
  - **누가 언제 되나요?(하단):** 참가자별로 고른 날짜를 칩으로 요약. 칩 색은 **날짜별
    인기순**(가장 많이 선택된 날짜=주황, 2위=노랑, 나머지=연노랑). 날짜 투표에
...
- **낙서 지속성/동기화 보완:**
  - 캔버스 PNG 스냅샷을 방에 저장 → 늦게 들어온 사람도 현재 그림을 봄.
  - 저장 시점: 펜 뗀 뒤(디바운스) + 주기 자동저장(4초) + 모바일 패널 닫을 때 + 탭
    백그라운드(visibilitychange) 시. 저장 실패 시 3회 재시도.
  - 채널이 아직 연결 전이거나 끊긴 동안 보낸 스트로크는 큐에 담아 (재)연결 시 전송해
    실시간 유실을 줄임. 채널은 마운트 즉시 구독 시작.
