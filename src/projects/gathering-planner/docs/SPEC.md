...
- **헤더:** 모임 이름, 접속 중인 다른 유저 닉네임 카드(겹친 스택, Realtime Presence),
  초대링크 복사 버튼, 내 닉네임.
- **낙서 보드:**
  - **PC:** 화면 전체를 덮는 캔버스 레이어 + 플로팅 툴바
    (조작(선택) / 그리기(펜) / 지우개 / 5색 팔레트: Red·Blue·Green·Yellow·Black).
  - **모바일:** 화면 전체 대신 왼쪽 하단의 **포스트잇 버튼(🖍️ 낙서장)** → 탭하면 하단
    시트로 **16:9 낙서 패널**이 열리고 거기서만 그림. (달력/장소는 평소처럼 바로 조작)
  - 두 경우 모두 같은 공유 레퍼런스 사각형에 그려서 PC·모바일 그림이 일치.
...
- **해상도 일관성:** 스트로크는 PC 비율(16:9) 기준의 공유 논리 사각형 좌표로 저장되고,
  각 기기에서 contain 방식으로 매핑됨. 그래서 모바일에선 늘어나지 않고 가운데 띠 형태로
  같은 모양이 보임(PC 기준).
