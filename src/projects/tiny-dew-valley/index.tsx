...
// Tiny Dew Valley 비주얼 토큰 — 양피지 패널 + 따뜻한 갈색 테두리 + 풀빛 액센트.
const MONO = "ui-monospace, 'SFMono-Regular', 'Courier New', monospace"
const UI = {
  panel: '#f6e9c9', panelSoft: '#fbf2da', card: '#fdf6e3',
  border: '#b88a52', borderSoft: '#cdb085', track: '#d9c39a',
  text: '#4a3422', accent: '#7cae4e', accentEdge: '#5a8a32',
  good: '#4f9d4a', bad: '#c64a3a', gold: '#a6791f',
}
const PARCH = 'rgba(246,233,201,0.96)'
...
export default function TinyDewValley() {
  ...
  return (
    <div style={{
      position: 'fixed', inset: 0, background: '#1c2a1c',
      display: 'flex', justifyContent: 'center', fontFamily: MONO, color: UI.text, touchAction: 'none', userSelect: 'none',
    }}>
      <div ref={wrapRef} style={{
        position: 'relative', width: 'min(100vw, 480px)', height: '100%',
        overflow: 'hidden', boxShadow: '0 0 40px rgba(0,0,0,0.4)', background: '#7cb7c9',
      }}>
        ...
        {questFlash && (
          <div style={{
            ...
            background: UI.good, color: '#fff', padding: '6px 14px', borderRadius: 10, fontWeight: 700, fontSize: 13, boxShadow: '0 2px 0 rgba(0,0,0,0.25)', zIndex: 20,
        ...
        <div style={{ position: 'absolute', top: 110, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, zIndex: 25, pointerEvents: 'none' }}>
          {toasts.map((t) => (
            <div key={t.id} style={{
              background: UI.panel, border: `2px solid ${UI.border}`,
              color: t.kind === 'good' ? UI.good : t.kind === 'bad' ? UI.bad : UI.text,
              padding: '5px 12px', borderRadius: 7, fontSize: 12, fontWeight: 700, boxShadow: '0 2px 0 rgba(0,0,0,0.2)',
            }}>{t.text}</div>
          ))}
        </div>
          ...
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 76, display: 'flex', justifyContent: 'center', gap: 8, zIndex: 18, padding: '0 8px', flexWrap: 'wrap' }}>
            {ctxActions.map((a) => (
              <button key={a.id} disabled={!a.enabled} onClick={() => game.doContext(a.id)} style={{
                ...
                border: `2px solid ${UI.border}`, borderRadius: 8, padding: '7px 13px', fontWeight: 800, fontSize: 14, fontFamily: MONO,
                opacity: a.enabled ? 1 : 0.7, boxShadow: '0 2px 0 rgba(0,0,0,0.2)', cursor: a.enabled ? 'pointer' : 'default',
              }}>
        ...
        {game.getMode() === 'mine' && !panel && (
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 16, display: 'flex', justifyContent: 'center', gap: 8, zIndex: 18 }}>
            ...
            <div style={{ ...mineBtn, background: '#3a2a1a', color: '#f6e9c9' }}>지하 {game.mineFloor}층</div>
        ...
        {game.getMode() === 'island' && (
          <div style={{
            position: 'absolute', left: 0, right: 0, bottom: 0, height: 64, background: PARCH,
        ...
        {celebrate && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 40, pointerEvents: 'none' }}>
            <div style={{
              background: '#fff8e8', border: `3px solid ${UI.border}`, borderRadius: 16, padding: '20px 26px',
              textAlign: 'center', color: UI.text, boxShadow: '0 6px 0 rgba(0,0,0,0.25)', animation: 'tdv-pop 0.4s ease',
            }}>
      ...
      <style>{`@keyframes tdv-pop {0%{transform:scale(0.6);opacity:0}100%{transform:scale(1);opacity:1}}
        .tdv-scroll::-webkit-scrollbar{width:8px}.tdv-scroll::-webkit-scrollbar-thumb{background:${UI.border};border-radius:4px}`}</style>
    </div>
  )
}

const mineBtn: React.CSSProperties = {
  background: UI.panel, color: UI.text, border: `2px solid ${UI.border}`, borderRadius: 8,
  padding: '8px 14px', fontWeight: 800, fontSize: 14, fontFamily: MONO, boxShadow: '0 2px 0 rgba(0,0,0,0.2)', cursor: 'pointer',
}
...
function Hud({ s }: { s: GameState }) {
  ...
  const barColor = pct <= 0 ? UI.bad : pct < 0.3 ? '#e0a93f' : UI.good
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '8px 10px', zIndex: 16, pointerEvents: 'none' }}>
      ...
      <div style={{ marginTop: 6, background: PARCH, borderRadius: 8, padding: '5px 10px', border: `2px solid ${UI.border}`, maxWidth: 230, boxShadow: '0 2px 0 rgba(0,0,0,0.18)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, fontWeight: 700, color: UI.text }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><PixelIcon name="bolt" size={14} /> 스태미나</span>
          <span>{s.stamina} / {s.maxStamina}</span>
        </div>
        <div style={{ marginTop: 3, height: 9, background: UI.track, borderRadius: 6, overflow: 'hidden' }}>
          <div style={{ width: `${pct * 100}%`, height: '100%', background: barColor, transition: 'width 0.2s' }} />
        </div>
      </div>
      {q && (
        <div style={{
          marginTop: 6, maxWidth: 300, background: 'rgba(58,42,26,0.88)', color: '#f6e9c9',
          border: '2px solid rgba(184,138,82,0.85)', boxShadow: '0 2px 0 rgba(0,0,0,0.25)', borderRadius: 8,
          ...
          <div style={{ marginTop: 4, height: 6, background: 'rgba(255,255,255,0.22)', borderRadius: 4, overflow: 'hidden' }}>
            <div style={{ width: `${Math.min(100, (qCurrent / Math.max(1, qTarget)) * 100)}%`, height: '100%', background: '#f0c84e' }} />
          </div>
        </div>
      )}
    </div>
...
function Pill({ children }: { children: React.ReactNode }) {
  return <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: PARCH, border: `2px solid ${UI.border}`, borderRadius: 8, padding: '4px 10px', fontWeight: 700, fontSize: 13, color: UI.text, boxShadow: '0 2px 0 rgba(0,0,0,0.18)' }}>{children}</div>
}
function MenuBtn({ icon, label, onClick }: { icon: string; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      background: UI.panel, border: `2px solid ${UI.border}`, borderRadius: 8, display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', gap: 2, color: UI.text, cursor: 'pointer', fontFamily: MONO,
      width: 52, height: 48, padding: 2, boxShadow: '0 2px 0 rgba(0,0,0,0.2)',
    }}>
    ...
    <div style={{ display: 'grid', gap: 10 }}>
      {entries.map((e) => (
        <button key={e.id} onClick={() => openPanel(e.id)} style={{
          display: 'flex', alignItems: 'center', gap: 12, background: UI.card, border: `2px solid ${UI.border}`, borderRadius: 9,
          color: UI.text, padding: '11px 12px', textAlign: 'left', boxShadow: '0 2px 0 rgba(0,0,0,0.18)', fontFamily: MONO, cursor: 'pointer',
        }}>
...
function Panel({ icon, title, onClose, children }: { icon: string; title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(30,24,16,0.5)', zIndex: 50, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: UI.panel, borderTopLeftRadius: 16, borderTopRightRadius: 16, border: `3px solid ${UI.border}`, borderBottom: 'none', maxHeight: '78%', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: `2px solid ${UI.border}` }}>
          ...
          <button onClick={onClose} style={{ background: UI.bad, color: '#fff', border: '2px solid #973225', borderRadius: 8, width: 30, height: 30, fontWeight: 800, cursor: 'pointer', fontFamily: MONO }}>✕</button>
        </div>
        <div className="tdv-scroll" style={{ overflowY: 'auto', padding: 14, color: UI.text }}>{children}</div>
      </div>
    </div>
  )
}
...
function btnStyle(enabled: boolean): React.CSSProperties {
  return { display: 'inline-flex', alignItems: 'center', gap: 4, background: enabled ? UI.accent : '#c2b79e', color: '#fff', border: `2px solid ${enabled ? UI.accentEdge : '#a89e85'}`, borderRadius: 8, padding: '6px 11px', fontWeight: 800, fontSize: 13, fontFamily: MONO, cursor: enabled ? 'pointer' : 'default', boxShadow: enabled ? '0 2px 0 rgba(0,0,0,0.2)' : 'none' }
}
function Row({ children }: { children: React.ReactNode }) {
  return <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px dashed #cdb085' }}>{children}</div>
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
      {s.inventory.map((e) => {
        ...
        return (
          <div key={e.itemId} style={{ background: UI.card, border: `2px solid ${UI.border}`, borderRadius: 9, padding: 8, textAlign: 'center' }}>
            <PixelIcon name={e.itemId} size={30} />
            <div style={{ fontSize: 11, fontWeight: 700 }}>{it?.name}</div>
            <div style={{ fontSize: 13, fontWeight: 800, color: UI.gold }}>×{e.count}</div>
          </div>
        )
      })}
    </div>
  ...
  return (
    <div>
      ...
      {active.map((q) => {
        ...
        return (
          <div key={q.id} style={{ background: UI.card, border: `2px solid ${UI.border}`, borderRadius: 9, padding: 10, marginBottom: 8 }}>
            <div style={{ fontWeight: 700 }}>{q.name}</div>
            <div style={{ fontSize: 12, opacity: 0.85 }}>{q.desc}</div>
            <div style={{ marginTop: 6, height: 8, background: UI.track, borderRadius: 5, overflow: 'hidden' }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button style={{ ...btnStyle(true), background: UI.bad, borderColor: '#973225' }} onClick={() => { if (confirm('정말 처음부터 다시 시작할까요? 모든 진행이 사라져요.')) game.resetGame() }}>초기화</button>
      </div>
...
function Slider({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <Row>
      ...
      <input type="range" min={0} max={1} step={0.05} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} style={{ flex: 1, accentColor: UI.accent }} />
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <div style={{ fontWeight: 800, fontSize: 14, color: UI.accentEdge, margin: '12px 0 4px' }}>{children}</div>
}
