...
export class GameRenderer {
  ...
  private drawFieldSigns(S: number) {
    for (const plot of FIELD_PLOTS) {
      ...
      const crop = rows > 0 ? this.selectedCropForSign(plot.id) : null
  ...
  private selectedCropForSign(fieldId: string): CropDef {
  ...
  // the field reads as a distinct little planting bed (all crisp pixels).
    ...
    for (const plot of FIELD_PLOTS) {
      for (let ty = plot.y; ty < plot.y + FIELD_SIZE; ty++) {
        for (let tx = plot.x; tx < plot.x + FIELD_SIZE; tx++) {
          ...
          // Per-cell bevel: lit top + left, shadowed bottom + right.
          r(0, 0, 16, 1, lit)
          r(0, 0, 1, 16, litS)
          r(0, 15, 16, 1, shade)
          r(15, 0, 1, 16, shade)
  ...
  private drawWorkPose(x: number, y: number, dir: string, t: number, S: number, tool: UpgradeableToolId | 'hand') {
    ...
    const handleTop = tool === 'scythe' ? -13 : tool === 'pickaxe' ? -13 : -10
    ...
    if (tool === 'sword') {
      ...
      ctx.fillRect(0, -15 * S, 9 * S, 2 * S)
      ctx.fillRect(7 * S, -14 * S, 2 * S, 4 * S)
      ctx.fillRect(-1 * S, -14 * S, 2 * S, 2 * S)
      ctx.fillStyle = metal.light
      ctx.fillRect(0, -16 * S, 8 * S, 1 * S)
      ctx.fillRect(7 * S, -15 * S, 2 * S, 1 * S)
      ctx.fillStyle = metal.dark
      ctx.fillRect(5 * S, -10 * S, 3 * S, 1 * S)
